...
export function FornaContainer(element, passedOptions = {}) {
    ...
    self.changeColorScheme = function(newColorScheme) {
        ...
        } else if (newColorScheme == 'strand') {
            // Create a color scale for strands
            var strandColors = d3.scale.ordinal()
                .range(['#ff7f0e', '#1f77b4']) // Orange for strand 1, Blue for strand 2
                .domain([1, 2]);

            nodes.style('fill', function(d) {
                if (d.nodeType !== 'nucleotide') return 'white';
                
                // Get the length of the first sequence
                let seq1Length = d.rna.seq.length / 2;  // Since the total sequence is concatenated
                
                // Determine strand based on position
                let strandNum = d.num <= seq1Length ? 1 : 2;
                return strandColors(strandNum);
            });
        }
    };
    ...
    self.addRNA = function(structure, passedOptions = {}) {
        ...
        // Add strand information to nodes if provided
        if ('strand' in passedOptions) {
            rnaJson.nodes.forEach(function(node) {
                if (node.nodeType === 'nucleotide') {
                    node.strand = passedOptions.strand;
                }
            });
        }
